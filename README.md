...
Desde 2025 conduz, como **product owner técnico**, projetos open-source de segurança e privacidade em Python, Go, Rust e Swift, com a codificação orquestrada por assistentes de IA generativa sob sua direção e revisão. O SentinelBR seguiu esse modelo: exercitando a tradução de exigências regulatórias (LGPD) e conceitos de threat detection em uma plataforma funcional. Também desenvolve **automações de processos com n8n** e é autor de cinco livros publicados, entre eles *Da Norma à Liderança*, sobre atualização profissional em GRC.
...
- **SC Platform** *(privado, disponível para apresentação mediante solicitação)* — SaaS multi-tenant de gestão de licitações públicas (PNCP em tempo real, simulador FSM da Lei 14.133, robô de lances em 3 modos, extração de PDF com IA local, CRM, Telegram). ~75k linhas, 547 testes. Python 3.14 + Flask 3 + PostgreSQL + Redis.
